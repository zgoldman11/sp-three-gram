            ...
            if (isArray) {
                ...
                if (dimCount === 0) {
                    ...
                    for (let j = 0; j < size; j++) {
                        ...
                        offset = elementEnd;
                        array[j] = read(elementType, elementStart, elementEnd);
                    }
                    value = array;
                }
                else {
        ...
        const addBinaryValue = (value, dataType) => {
            ...
            switch (dataType) {
                ...
                case types_1.DataType.Timestamptz:
                    {
                        if (value === infinity) {
                            size = utils_1.sum(add(6 /* UInt32BE */, 0x7fffffff), add(6 /* UInt32BE */, 0xffffffff));
                        }
                        else if (value === -infinity) {
                            size = utils_1.sum(add(6 /* UInt32BE */, 0x80000000), add(6 /* UInt32BE */, 0x00000000));
                        }
                        else if (value instanceof Date) {
                            const n = (value.getTime() - timeshift) * 1000;
                            const f = Math.floor(n / 4294967296);
                            const r = n - f * 4294967296;
                            size = utils_1.sum(add(5 /* Int32BE */, f), add(6 /* UInt32BE */, r));
                        }
                        break;
                    }
                    ;
